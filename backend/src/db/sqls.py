...
(   SELECT 
    ...
    FROM 
    (   
		select * from stories_changelogs where log_id = 1 and story_complete_time is not null
	) t
    JOIN 
        holidays di 
    ON 
...
t_sprint_seqno AS (select project_id, sprint_id, short_sprint_name, row_number() over(partition by project_id order by sprint_id) sprint_seqno from rdmdb.sprints),
t_main AS (
            select 
                t1.project_id,
            	t1.project_name, 
