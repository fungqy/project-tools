...
def gitlab_get_group_projects(group_key):
    ...
    return [p for p in projects if "release" != p["name"] and "project-doc" != p["name"] and "repo-lib" != p["name"] and "alg" != p["name"] and "mowing-web" != p["name"] and "md-docs-web" != p["name"]]
...
def gitlab_get_project_commitdate(project_id):
    ...
    if branches:
        for branch in branches:
            ...
            if branch_commitdate is not None and (latest_commitdate is None or branch_commitdate > latest_commitdate):
                latest_commitdate = branch_commitdate
    return datetime.strptime(latest_commitdate[0:10], '%Y-%m-%d').date() if latest_commitdate else None
...
def sonar_check_project_exists(project_key):
    """检查 SonarQube 项目是否存在"""
    try:
        response = requests.get(PROJECTS_API, params={"projects": project_key}, headers={"Authorization": f"Bearer {SONAR_TOKEN}"})
...
def sonar_get_project_branches(project_key):
    ...
    if _res.status_code == 200:
        ...
        print(f"[Warning] 获取项目'{project_key}' 的分支失败: {_res.status_code}, {_res.text}")
        return None
...
def sonar_get_project_scandate(project_branches):
    ...
    for branch in project_branches:
        ...
        if branch_analysisdate is not None and (latest_scan_date is None or branch_analysisdate > latest_scan_date):
            latest_scan_branch = branch
            latest_scan_date = branch_analysisdate
    # 改成北京时间
    if latest_scan_date:
        latest_scan_date = (datetime.strptime(latest_scan_date[0:19], '%Y-%m-%dT%H:%M:%S') + timedelta(hours=8)).date()
    return latest_scan_branch, latest_scan_date
...
def sonar_get_branch_bug_cnt(project_key, branch_name, islatest="false"):
    ...
    if response.status_code == 200:
        ...
        print(f"[Warning] 查询项目 {project_key} 分支 {branch_name} 的BUG数失败'': {response.status_code}")
        return None
...
def sonar_projects_bug_count(projects):
    ...
    for item in projects:
        ...
        bugs_cnt = sonar_get_project_bug_cnt(sonar_project_key, branches) if branches else 0
        if bugs_cnt:
...
def gene_message(config: ProjectRemindConfig):
    ...
    if gitlab_group_key == "zhks":
        gitlab_group_projects = [item for item in gitlab_group_projects if item['name'] in ZHKS_SONAR_KEY_MAP.keys()]

    gitlab_group_projects = [(project, sonar_get_project_key(gitlab_group_key, config.sonar_key_prefix, project['name'])) for project in gitlab_group_projects]
    ...
    for item in gitlab_group_projects:
        ...
        commit_in_this_week = dateattr.firstday_of_week <= latest_commit_date <= dateattr.lastday_of_week if latest_commit_date else False
        ...
        latest_scan_branch_bug_cnt = sonar_get_branch_bug_cnt(sonar_project_key, latest_scan_branch, "true")
        scan_in_this_week = dateattr.firstday_of_week <= latest_scan_date <= dateattr.lastday_of_week if latest_scan_date else False

        date_message = f"(最近提交日期 {latest_commit_date}, 最近扫描日期 {latest_scan_date})"
        scan_message = "<font color=\"warning\">从未扫描</font>" if not latest_scan_date else ("<font color=\"warning\">未扫描</font>" if commit_in_this_week and not scan_in_this_week else "")
        bug_message = f"<font color=\"warning\">, 且存在{latest_scan_branch_bug_cnt}个缺陷</font>" if latest_scan_branch_bug_cnt > 0 and scan_message else (f"<font color=\"warning\">存在{latest_scan_branch_bug_cnt}个缺陷</font>" if latest_scan_branch_bug_cnt > 0 else "")

        if gitlab_group_key == "cmp":
            project_manager = " ".join([f"<@{manager}>" for manager in CMP_PROJECTS_MANAGER.get(project_name, [])]) if CMP_PROJECTS_MANAGER.get(project_name, []) else sonar_remind_person
        else:
            project_manager = sonar_remind_person

        if scan_message + bug_message:
            message += "> " + f"<font color=\"blue\">{project_name}</font>" + " " + scan_message + bug_message + " " + date_message + " " + project_manager + "\n"
    if message:
        message = "## SONAR 扫描情况" + "\n" + message
    return message
